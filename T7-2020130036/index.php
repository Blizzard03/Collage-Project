...
if ($Operation == 'delete') {
    ...
    if ($q1) {
        ...
        $Gagal  = "404!!! Gagal melakukan delete data";
    }
}
if ($Operation == 'edit') {
    $id         = $_GET['id'];
    ...
    if ($judul == '') {
        $Gagal = "404!!! Data tidak ditemukan";
    }
}
if (isset($_POST['simpan'])) { //untuk create
    ...
    if ($judul && $halaman && $rating && $kategori) {
        if ($Operation == 'edit') { //untuk update
            $sql1       = "update buku set judul = '$judul',halaman='$halaman',rating = '$rating',kategori='$kategori' where id = '$id'";
            $q1         = mysqli_query($Database_konektor, $sql1);
            if ($q1) {
                $Berhasil = "Data berhasil diupdate";
            } else {
                $Gagal  = "404!!! Data gagal diupdate";
            }
        } else { //untuk insert
            $sql1   = "insert into buku(judul,halaman,rating,kategori) values ('$judul','$halaman','$rating','$kategori')";
            $q1     = mysqli_query($Database_konektor, $sql1);
            if ($q1) {
                $Berhasil     = "Berhasil input data baru";
            } else {
                $Gagal      = "404!!! Gagal input data";
            }
        }
    } else {
        $Gagal = "Silakan input semua data";
    }
}
?>
<!DOCTYPE html>
